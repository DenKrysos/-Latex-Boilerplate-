...
% \newcommand{\orcidIco}[1]{\href{https://orcid.org/#1}{\includesvg[height=1.5ex]{\DenKrAllMainRootDir/4aids/graphics/logo/orcid.logo.icon}}}%
% \newcommand{\orcidIcoText}[1]{\href{https://orcid.org/#1}{\includegraphics[height=1.5ex]{\DenKrAllMainRootDir/4aids/graphics/logo/orcid.logo.icon.eps} \hspace{1.5em} orcid.org/#1}}%
% Using Fontawesom5
\newcommand{\orcidLogo}{\raisebox{0.2ex}[0pt][0pt]{\resizebox{!}{1.4ex}{\textcolor{orcidlogocol}{\faIcon{orcid}}}}}%
\newcommand{\orcidIco}[1]{\href{https://orcid.org/#1}{\orcidLogo}}%
\newcommand{\orcidIcoText}[1]{\href{https://orcid.org/#1}{\orcidLogo \hspace{1.0em} orcid.org/#1}}%
\newcommand{\orcidIcoURL}[1]{\href{https://orcid.org/#1}{\orcidLogo\hspace{0.35ex}https://orcid.org/#1}}% Space 1/4 of the logo-width
%
%
%
%
...
%       3: With Stretching factor. Then you supply a second Value in the Argument, seperated by comma, which is multiplied with the max possible width to become the pictures width. ("max possible width" means the amount allocated for the single picture, not as a share of a total Line-Width)
%          -> e.g. [3,0.7]
%       [3, <Stretch-Factor>, <OneLineExtend>]: If '3', it can have a third argument. This is to decide how to handle the special-case, when the "Matrix" only has 1 Line. When you include some pictures, all in one line, witch a Stretch-Factor, so that they won't sum up to fill the entire \linewidth.
%          - Default: The Frame is drawn, to snug on the images (possibly leaving blank space left/right).
%          - Adding a 3rd Argument (of any value, just have something): The Frame is extended to fill the entire line, having blank space INSIDE the Frame.
%          -> e.g. [3,0.7,1]
%       4: ToDo - Use the height to dictate the individual picture-size in the case of One-Line
%       5: Width as relative Size-Indicator, but scaling up to height, if horizontal space remaining. The second Argument behind the "[5,]" gives a Factor that is multiplied with the maximum size, a single image can get (i.e. Textwidth / Num-Pics).
%            As second step, available horizontal space is used to scale Images up to the height, the tallest Image has. If not enough horizontal space is free, partial scaling is done.
%       6: Width as relative Size-Indicator, but bringing all to same height, as to how available. The second Argument behind the "[5,]" gives a Factor that is multiplied with the maximum size, a single image can get (i.e. Textwidth / Num-Pics).
%            As second step, all Images are scaled-up to the height, the tallest Image has. If this exceeds the Textwidth, all are proportionally scaled down. This should make the best use of the available space.
%          -> e.g. [5,0.7]
%
...
\DeclareDocumentCommand{\dishPicMat}{%
	O{2} O{0} >{\SplitList{|}}m >{\SplitArgument{2}{,}}O{0}%
}{%
	%\newcounter{quotient}%
	...
	\ifnumcomp{\value{numArgs}}{<}{\value{numPerLine}}{%
		\setcounter{numPerLine}{\value{numArgs}}%
	}{}%
	...
	\let\innAbst\undefined\newlength\innAbst\setlength\innAbst{1.5ex}% Abstand zwischen Text und Zellenrand
	\let\lineHeight\undefined\newlength\lineHeight\setlength\lineHeight{0pt}%
	\let\heightCmp\undefined\newlength\heightCmp\setlength\heightCmp{0pt}%
	\let\widthCmp\undefined\newlength\widthCmp\setlength\widthCmp{0pt}%
	\let\nodeWidth\undefined\newlength\nodeWidth%
	...
	\begin{tikzpicture}%
		...
		\loop\ifnum\value{loopi}<\value{numArgs}%
			...
			% The last line (ore there is only 1 Line)
			% \ifthenelse{\number\stretchArgMode=1 \AND \thenumInLastLine<\thenumPerLine}{%
			\ifthenelse{\thenumInLastLine<\thenumPerLine \AND \thecurrLine=\thenumLines \OR \(1=\thenumLines\)}{%
				...
				}{\ifthenelse{\number\stretchArgMode=5}{%
					\setlength\picWidth{\nodeWidth-2\innAbst}%
					\setlength\picWidth{\stretchArgFactor\picWidth}%
				}{}}}}%
			}{}%
			% Special-Case: When there is only 1 Line and the Options do not tell to extend this, set the minimum width of a node to be drawn to 0
			\ifthenelse{1=\thenumLines \AND \number\oneLineFill=0 \AND \(\number\stretchArgMode=3 \OR \number\stretchArgMode=5\)}{%
				\renewcommand{\nodeDrawWidth}{0pt}%
			}{}%
			...
			\ifnumcomp{\thecurrRow}{=}{1}{%
				...
				{\loop\ifnum\value{loopj}<\value{temp}%Inner loops must be embraced
					...
					\settoheight{\heightCmp}{\includegraphics[width=\srcPicWidth]{\checkArrayEnt{\theloopj}{}{\getArrayEnt{\theloopj}}}}\global\heightCmp=\heightCmp%
					\ifnumcomp{\heightCmp}{>}{\picHeight}{%
						\setlength\picHeight{\heightCmp}\global\picHeight=\picHeight%
					}{}%
					\setlength\lineHeight{\picHeight+2\innAbst}\global\lineHeight=\lineHeight%
				\stepcounter{loopj}%
				\repeat}%
				% In case of Stretch-Mode 5, see if available horizontal space can be used to scale up Images of lower height
				\ifthenelse{1=\thenumLines \AND \number\stretchArgMode=5}{%
					\setcounter{loopj}{\theloopi}%
					\setcounter{temp}{\theloopj+\thenumInLastLine}%
					\setlength\widthCmp{0pt}
					\let\widthSumScaled\undefined\newlength\widthSumScaled\setlength\widthSumScaled{0pt}%
					\let\widthSumOrig\undefined\newlength\widthSumOrig\setlength\widthSumOrig{0pt}%
					\let\reducedWidth\undefined\newlength\reducedWidth\setlength\reducedWidth{\linewidth-\recipeBorderWidthDishPic}%
					{\loop\ifnum\value{loopj}<\value{temp}%
						\dishPicMatSetMaxPicDimension{\theloopj}%
						\settoheight{\heightCmp}{\includegraphics[width=\srcPicWidth]{\checkArrayEnt{\theloopj}{}{\getArrayEnt{\theloopj}}}}%
						\ifnumcomp{\heightCmp}{<}{\picHeight}{%
							\settowidth{\widthCmp}{\includegraphics[height=\picHeight]{\checkArrayEnt{\theloopj}{}{\getArrayEnt{\theloopj}}}}%
							% A bit misusing a variable
							\settowidth{\heightCmp}{\includegraphics{\checkArrayEnt{\theloopj}{}{\getArrayEnt{\theloopj}}}}%
							\ifnumcomp{\heightCmp}{<}{\widthCmp}{%
								\setlength{\widthCmp}{\heightCmp}%
							}{}%
							\setlength{\widthSumScaled}{\widthSumScaled+\widthCmp}\global\widthSumScaled=\widthSumScaled%
							\setlength{\widthSumOrig}{\widthSumOrig+\srcPicWidth}\global\widthSumOrig=\widthSumOrig%
							\setlength{\reducedWidth}{\reducedWidth-2\innAbst-\recipeBorderWidthDishPic}\global\reducedWidth=\reducedWidth%
						}{%
							% That's the tallest, don't include in calculation
							\setlength{\reducedWidth}{\reducedWidth-(\srcPicWidth+2\innAbst+\recipeBorderWidthDishPic)}\global\reducedWidth=\reducedWidth%
						}%
					\stepcounter{loopj}%
					\repeat}%
					\tikzmath{%
						if \widthSumOrig > 0 then {%
							\factorIs=\widthSumScaled/\widthSumOrig;%
							\factorWant=\reducedWidth/\widthSumOrig;%
							\upscaleBoundaryFactor=\factorWant/\factorIs;%
							if \upscaleBoundaryFactor > 1 then {%
								\upscaleBoundaryFactor=1.0;%
							};%
						} else {%
							\upscaleBoundaryFactor=1.0;%
						};%
			...
			\ifthenelse{\number\stretchArgMode=5 \AND 1=\thenumLines}{%
				% Individual handling for the Streching-Mode 5
				\settoheight{\heightCmp}{\includegraphics[width=\srcPicWidth]{\checkArrayEnt{\theloopi}{}{\getArrayEnt{\theloopi}}}}%
				\ifnumcomp{\heightCmp}{<}{\picHeight}{%
					\settowidth{\widthCmp}{\includegraphics[height=\upscaleBoundaryFactor\picHeight]{\checkArrayEnt{\theloopi}{}{\getArrayEnt{\theloopi}}}}%
					\settowidth{\srcPicWidth}{\includegraphics{\checkArrayEnt{\theloopi}{}{\getArrayEnt{\theloopi}}}}%
					\ifnumcomp{\widthCmp}{<}{\srcPicWidth}{%
						\setlength{\srcPicWidth}{\widthCmp}%
					}{}%
				}{%
					% That's the tallest, do nothin'
				}%
			}{}%
			\begin{pgfonlayer}{Layer2}%
		...
		\stepcounter{loopi}%
		\repeat%
		\let\nodeDrawWidth\undefined%
		\let\nodeDrawHeight\undefined%
	\end{tikzpicture}%
	% \end{center}
	\let\stretchArgMode\undefined%
	\let\stretchArgFactor\undefined%
	\let\oneLineFill\undefined%
...
\NewDocumentCommand{\dishPicMatParseArgStretch}{mmm}{%
...
\begin{pgfonlayer}{Layer1}%
	...
	\path[pathLineFill]($(#1.north)+(0.1pt,0)$){[#3]-|(#1.east)}{[#4]|-(#1.south)}{[#5]-|(#1.west)}{[#2]|-($(#1.north)+(-0.1pt,0)$)}--cycle;%
\end{pgfonlayer}%
}%
...
%vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
%____________________________________________________________________
% When you compile a Document occasionally on DIN-A4 and also DIN-A5 paper, normally included graphics would appear smaller on the DIN-A5, when including with a Factor to e.g. the line/text/column-width.
% The following Macro sets you the variable "\InclGraphWidthDINA" to an optimized value for both scenarios.
% USAGE Example: \setInclGraphWidthDINA{0.7}% Set the given value to the desired Factor of the \linewidth
%   - When you operate on a DIN-A4 paper, it sets \InclGraphWidthDINA to 0.7 the \linewidth of this DIN-A4 paper
%   - When a DIN-A5 paper is compiled, is increases the Factor accordingly, to keep a graphic the same visual size for the reader (and accordingly relatively bigger to the DIN-A5 paper width).
%   -> While doing so, it assures that the Factor never goes beyond 1.0
\newcommand{\setInclGraphWidthDINA}[1]{%
\def\InclGraphWidthDINAFiveMult{1.4}% A5 Paper Format textwidth is decreased by Factor of sqrt(2)
\edef\InclGraphWidthDINAFactorPic{#1}%
\edef\InclGraphWidthDINAFactor{\InclGraphWidthDINAFactorPic}%
\expandafter\ifstrequal\expandafter{\DenKrLayout}{scrbook_print}{%
	\edef\InclGraphWidthDINAFactor{\fpeval{\InclGraphWidthDINAFiveMult*\InclGraphWidthDINAFactor}}%
}{}%
\ifthenelse{1=\fpeval{\InclGraphWidthDINAFactor>1.0}}{%
	\edef\InclGraphWidthDINAFactor{1}%
}{}%
\edef\InclGraphWidthDINA{\InclGraphWidthDINAFactor\linewidth}%
}%
%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
%------------- DIN-A Paper END -------------
%====================================================================
